...
private:
    ...
    bool isEmpty() { return m_heap == nullptr; }
    //used to merge two trees together
    Node* merge(Node* root1, Node* root2);
    //a recursive clear function for the clear non recursive function
    Node* clearRecur(Node* root);
    //recursive function for mergeWithQueue min edition
    Node* mergeQueueMin(Node* heap1, Node* heap2);
    //recursive function for mergeWithQueue max edition
    Node* mergeQueueMax(Node* heap1, Node* heap2);
    //recursion for the copy constructor and assignment operator
    Node* copyConstructRecur(Node* rhsHeap);
    //recursion for printing the crops in the queue
    void printRecur(Node* root) const;
    //copies from the first heap and adds it into the other heap
    Node* priorityChange(Node* currHeap,IQueue& heap1);
   };
#endif